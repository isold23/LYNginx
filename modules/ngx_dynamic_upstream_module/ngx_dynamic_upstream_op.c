...
static const ngx_str_t ngx_dynamic_upstream_params[] = {
    ...
    ngx_string("arg_up"),
    ngx_string("arg_down"),
    ngx_string("arg_reverse_proxy_type"),
    ngx_string("arg_reverse_proxy_value"),
    ngx_string("arg_update"),
    ngx_string("arg_group"),
    ngx_string("arg_reload"),
    ngx_string("arg_replace"),
};
...
ngx_dynamic_upstream_op_update_param(ngx_http_request_t* r, ngx_dynamic_upstream_op_t* op,
                                     ngx_slab_pool_t* shpool, ngx_http_upstream_srv_conf_t* uscf);
static ngx_int_t
ngx_dynamic_upstream_op_replace(ngx_http_request_t* r, ngx_dynamic_upstream_op_t* op,
                            ngx_slab_pool_t* shpool, ngx_http_upstream_srv_conf_t* uscf);
static ngx_int_t
ngx_dynamic_upstream_is_shpool_range(ngx_http_request_t* r, ngx_slab_pool_t* shpool, void* p)
{
...
ngx_dynamic_upstream_build_op(ngx_http_request_t* r, ngx_dynamic_upstream_op_t* op)
{
    ...
    for(i = 0; i < args_size; i++) {
        ...
        if(!var->not_found) {
            if(ngx_strcmp("arg_upstream", args[i].data) == 0) {
            ...
            } else if(ngx_strcmp("arg_replace", args[i].data) == 0) {
                op->op |= NGX_DYNAMIC_UPSTEAM_OP_REPLACE;
            } else if(ngx_strcmp("arg_update", args[i].data) == 0) {
                op->op |= NGX_DYNAMIC_UPSTEAM_OP_PARAM;
            } else if(ngx_strcmp("arg_reload", args[i].data) == 0) {
                ...
                char command[1124];
                snprintf(command, 1124, "%s -s reload", path);
                system(command);
                ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,
                              "reload nginx: %s",
                              command);
                op->op |= NGX_DYNAMIC_UPSTEAM_OP_RELOAD;
                return NGX_OK;
                ...
                op->verbose = 1;
            } else if(ngx_strcmp("arg_group", args[i].data) == 0) {
                op->group = ngx_atoi(var->data, var->len);
                
                if(op->group == NGX_ERROR) {
                    op->status = NGX_HTTP_BAD_REQUEST;
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                                  "group is not number. %s:%d",
                                  __FUNCTION__,
                                  __LINE__);
                    return NGX_ERROR;
                }
                
                op->op_param |= NGX_DYNAMIC_UPSTEAM_OP_PARAM_GROUP;
                op->verbose = 1;
                ...
                op->verbose = 1;
            } else if(ngx_strcmp("arg_reverse_proxy_type", args[i].data) == 0) {
                op->reverse_proxy_type.data = var->data;
                op->reverse_proxy_type.len = var->len;
                op->op_param |= NGX_DYNAMIC_UPSTREAM_OP_PARAM_REVERSE_PROXY_TYPE;
                op->verbose = 1;
                ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                              "reverse proxy type[%d:%s]", var->len, op->reverse_proxy_type.data);
            } else if(ngx_strcmp("arg_reverse_proxy_value", args[i].data) == 0) {
                op->reverse_proxy_value.data = var->data;
                op->reverse_proxy_value.len = var->len;
                op->op_param |= NGX_DYNAMIC_UPSTREAM_OP_PARAM_REVERSE_PROXY_VALUE;
                op->verbose = 1;
                ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                              "reverse proxy value: [%d:%s]", var->len, var->data);
            }
        }
    }
    ...
    if(op->up && op->down) {
        op->status = NGX_HTTP_BAD_REQUEST;
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "down and up at once are not allowed. up: %d, down: %d %s:%d",
                      op->up,
                      op->down,
                      __FUNCTION__,
                      __LINE__);
...
ngx_int_t
ngx_dynamic_upstream_op(ngx_http_request_t* r,
                        ...
                        ngx_http_upstream_srv_conf_t* uscf)
{
    ...
    case NGX_DYNAMIC_UPSTEAM_OP_REPLACE:
        rc = ngx_dynamic_upstream_op_replace(r, op, shpool, uscf);
        break;
...
static ngx_int_t
ngx_dynamic_upstream_op_replace(ngx_http_request_t* r,
                            ngx_dynamic_upstream_op_t* op,
                            ngx_slab_pool_t* shpool,
                            ngx_http_upstream_srv_conf_t* uscf)
{
    ...
    for(peer = peers->peer; peer; peer = peer->next) {
        ...
        ngx_strncmp(op->server.data, peer->name.data, peer->name.len) == 0) {
            found = 1;
            op->op = NGX_DYNAMIC_UPSTEAM_OP_LIST;
            op->op |= NGX_DYNAMIC_UPSTEAM_OP_PARAM; 
            rc = ngx_dynamic_upstream_op_update_param(r, op, shpool, uscf);
...
static ngx_int_t
ngx_dynamic_upstream_op_add(ngx_http_request_t* r,
                            ngx_dynamic_upstream_op_t* op,
                            ngx_slab_pool_t* shpool,
                            ngx_http_upstream_srv_conf_t* uscf)
{
    ...
    if(op->op_param & NGX_DYNAMIC_UPSTEAM_OP_PARAM_GROUP) {
        last->next->group = op->group;
    } else {
        last->next->group = 0;
    }
...
static ngx_int_t
ngx_dynamic_upstream_op_update_param(ngx_http_request_t* r,
                                     ngx_dynamic_upstream_op_t* op,
                                     ngx_slab_pool_t* shpool,
                                     ngx_http_upstream_srv_conf_t* uscf)
{
    ...
    if((op->op_param & NGX_DYNAMIC_UPSTREAM_OP_PARAM_REVERSE_PROXY_TYPE) ||
            (op->op_param & NGX_DYNAMIC_UPSTREAM_OP_PARAM_REVERSE_PROXY_VALUE)) {
        ...
        //ctx = rpcf->ctx;
        ctx->reverse_proxy_type.len = op->reverse_proxy_type.len;
        ngx_memcpy(ctx->reverse_proxy_type.data, 
        op->reverse_proxy_type.data, 
        op->reverse_proxy_type.len);
        ctx->reverse_proxy_value.len = op->reverse_proxy_value.len;
        ngx_memcpy(ctx->reverse_proxy_value.data, 
        op->reverse_proxy_value.data, 
        op->reverse_proxy_value.len);
        ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,
                      "server: %V ctx name: %V type: %V:%d value: %V:%d op type: %V:%d value: %V:%d",
                      &op->server, &ctx->name,
                      &ctx->reverse_proxy_type,
                      ctx->reverse_proxy_type.len,
                      &ctx->reverse_proxy_value,
                      ctx->reverse_proxy_value.len,
                      &op->reverse_proxy_type,
                      op->reverse_proxy_type.len,
                      &op->reverse_proxy_value,
                      op->reverse_proxy_value.len);
